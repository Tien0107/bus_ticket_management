...
import { OperatorPageShell, StatCard, LoadingState, ErrorState, ModalShell, SearchInput, SecondaryButton, PrimaryButton } from "../operator/OperatorUI";
...
export default function SupportTickets() {
  ...
  return (
    <OperatorPageShell
      eyebrow="Support"
      title="Danh sách vé"
      description="Tra cứu, kiểm tra trạng thái thanh toán và hỗ trợ hủy vé khi cần."
      actions={
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          ...
          <PrimaryButton icon="search" onClick={handleSearch}>Tìm</PrimaryButton>
          <SecondaryButton icon="refresh" onClick={handleRefresh}>Làm mới</SecondaryButton>
        </div>
      }
    >
      {/* Stats */}
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard icon="confirmation_number" label="Tổng số vé" value={stats.total} tone="primary" />
        <StatCard icon="pending_actions" label="Chờ xử lý" value={stats.pending} tone="amber" />
        <StatCard icon="payments" label="Đã thanh toán" value={stats.paid} tone="emerald" />
        <StatCard icon="cancel" label="Đã hủy" value={stats.cancelled} tone="red" />
      </div>

      {/* Filters */}
      <section className="mt-4 rounded-xl border border-outline-variant/30 bg-white p-4 shadow-sm">
        <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
          <div>
            <h3 className="text-base font-extrabold text-on-surface">Bộ lọc vận hành</h3>
      ...
      {/* Table */}
      <section className="mt-4 overflow-hidden rounded-xl border border-outline-variant/30 bg-white shadow-sm">
        <div className="flex items-center justify-between border-b border-surface-container px-4 py-2.5">
          <div>
            <h3 className="text-lg font-extrabold text-on-surface">Lịch sử đặt vé</h3>
        ...
        {error ? (
          <div className="m-5"><ErrorState message={error} /></div>
        ) : null}

        {loading && tickets.length === 0 ? (
          <div className="p-6"><LoadingState label="Đang tải dữ liệu vé..." /></div>
      ...
      {viewTicket ? (
        <ModalShell
          title={viewTicket.code || "Chi tiết vé"}
          subtitle="Thông tin chi tiết vé đặt chỗ"
          onClose={handleCloseModal}
          footer={
            <div className="flex justify-end gap-3">
              <SecondaryButton onClick={handleCloseModal}>Đóng</SecondaryButton>
        ...
        >
          {viewLoading ? (
            <div className="mb-4"><LoadingState label="Đang tải chi tiết vé..." /></div>
          ) : null}
          ...
          {actionError ? (
            <div className="mb-5"><ErrorState message={actionError} /></div>
          ) : null}
        </ModalShell>
      ) : null}
