...
export default function CustomerRegisterForm() {
  ...
  // Verification state for email and phone (customer simplified: only check availability, no OTP)
  ...
  // Wrappers for VerifiedContactField: editing after check clears the checked flag.
  // During verificationPhase (after clicking Đăng ký), we lock editing.
  ...
  const performSignUp = async () => {
    ...
    try {
      ...
      // Allow user to potentially retry verification? For now keep phase
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      // 1. Ensure availability right before sending OTPs (check button is optional helper, this is the real gate)
      ...
      // 2. Enter verification phase (this makes the contact fields render OTP UI + modals)
      setVerificationPhase(true);

      // 3. Send OTP for email first (phone will be auto-sent after email is successfully verified)
      await sendOtp({ field: "email", value: emailVal });
      setEmailVer((s) => ({ ...s, sent: true, otp: "", error: "" }));

      // The VerifiedContactField (with requireOtpVerification=true now) will auto-open the email OTP modal.
      // After successful email verify → handleVerifyOtp will send phone OTP and open phone modal.
      // After successful phone verify → it will call performSignUp().
    } catch (err) {
      ...
      // Reset checked flags so user can adjust if needed
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      {/* During verificationPhase the real "submit" is driven by successful OTP verifies */}
      <button
        type="submit"
