...
export default function SupportRegisterForm() {
  ...
  const handleCheck = async (field) => {
    ...
    if (!value) { setter((s) => ({ ...s, error: `Vui lòng nhập ${field === "email" ? "email" : "số điện thoại"}.` })); return; }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div className="space-y-2" ref={companyDropdownRef}>
        <label className="block text-sm font-medium text-on-surface-variant ml-1">Chọn công ty hỗ trợ</label>
      ...
      <button
        ...
        <span>Đăng ký nhân viên hỗ trợ</span>
        }
      </button>
    </form>);
