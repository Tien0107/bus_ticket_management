...
const Home = () => {
  ...
  const [companies, setCompanies] = useState([]);// quản lý danh sách các nhà xe đối tác
  const [companiesLoading, setCompaniesLoading] = useState(true);
  const [companiesError, setCompaniesError] = useState(null);

  const [popularRoutes, setPopularRoutes] = useState([]);//quản lý danh sách các tuyến đường phổ biến 
  const [loadingPopular, setLoadingPopular] = useState(false);

  const [promotions, setPromotions] = useState([]);// quản lý danh sách các chương trình khuyết mãi
      ...
      try {
        ...
        const companiesWithRatings = await Promise.all(
          baseCompanies.map(async (company) => {
            try {
              const res = await getTripScheduleRatings({
            ...
            if (!routesMap.has(key)) {
              routesMap.set(key, {
                ...
                duration: trip.durationMinutes
                  ? `Khoảng ${Math.floor(trip.durationMinutes / 60)}h ${trip.durationMinutes % 60}m`
                  : "Khoảng 2-3 giờ",
              });
            }
          });
        ...
        setPopularRoutes([
          {
            ...
            to: "Sapa",
            title: "Hà Nội ➔ Sapa",
            image: "https://picsum.photos/seed/hn-sp/800/1000",
            price: "Từ 250.000đ",
          ...
          },
          {
            ...
            image: "https://picsum.photos/seed/hcm-dl/800/1000",
            price: "Từ 300.000đ",
          ...
          },
          {
            ...
            image: "https://picsum.photos/seed/dn-ha/800/1000",
            price: "Từ 100.000đ",
          ...
          },
          {
            ...
            image: "https://picsum.photos/seed/hn-hp/800/1000",
            price: "Từ 120.000đ",
  ...
  return (
    <div className="min-h-screen bg-surface text-on-surface font-body">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-7">
            ...
            <p className="text-white/90 text-lg md:text-xl font-medium leading-relaxed max-w-xl">
              Tìm và đặt vé xe nhanh chóng, tiện lợi, giá tốt nhất cho hành trình khám phá Việt Nam
              của bạn.
            </p>
          </div>
            ...
            <div className="bg-white p-8 rounded-3xl shadow-editorial">
              <div className="space-y-6">
                ...
                <div className="relative space-y-4">
                  {}
                  <div
                    className={`transition-all duration-300 ${swapAnimating ? "translate-y-1 opacity-80" : ""}`}
                  >
                  ...
                  >
                    <span
                      className={`material-symbols-outlined text-[23px] transition-transform duration-300 ${swapAnimating ? "rotate-180" : ""}`}
                    >
                      swap_vert
                    </span>
                  </button>
        ...
        <section id="search-results" className="py-16 bg-surface">
          <div className="max-w-7xl mx-auto px-6">
            ...
            {searchError && !loadingSearch && (
              <div className="bg-red-50 text-red-600 p-4 rounded-xl text-center">{searchError}</div>
            )}
            ...
            {!loadingSearch && !searchError && schedules.length === 0 && (
              <div className="text-center py-10 text-on-surface-variant font-medium">
                ...
                Không tìm thấy chuyến xe nào phù hợp. Vui lòng thử thay đổi điểm đi/đến hoặc ngày
                đi.
              </div>
            )}
            ...
            {!loadingSearch && !searchError && schedules.length > 0 && (
              <div className="space-y-4">
                {schedules.map((schedule, idx) => (
                  <div
                    key={schedule.id || idx}
                    className="bg-white border text-left border-outline-variant/20 p-6 rounded-2xl shadow-sm hover:shadow-md transition-shadow flex flex-col md:flex-row justify-between items-center gap-6"
                  >
                        ...
                        <div className="flex items-center gap-3">
                          {schedule.logoUrl ? (
                            <img
                              ...
                              alt="Logo"
                              className="w-10 h-10 rounded-lg object-contain border border-outline-variant/20"
                            />
                          ) : (
                          ...
                          <div>
                            <p className="font-bold text-on-surface">
                              {schedule.name || "Chuyến xe"}
                            </p>
                            <p className="text-xs text-on-surface-variant flex items-center gap-1">
                              <span className="material-symbols-outlined text-[14px]">call</span>{" "}
                              {schedule.hotline || "Đang cập nhật"}
                            </p>
                          </div>
                        </div>
                      ...
                      <div className="flex items-center justify-between mt-4 relative">
                        ...
                        <div className="flex-1">
                          <p className="font-extrabold text-xl text-on-surface text-primary">
                            {schedule.departureTime ? schedule.departureTime.slice(0, 5) : "??:??"}
                          </p>
                          <p className="text-sm text-on-surface-variant font-medium mt-1">
                            {schedule.fromLocation || departure || "Điểm đi"}
                          </p>
                        </div>
                        {}
                        ...
                        <div className="flex-1 text-right">
                          <p className="font-extrabold text-xl text-on-surface text-primary">
                            {(() => {
                              ...
                              let d = new Date();
                              d.setHours(h);
                              d.setMinutes(m + schedule.durationMinutes);
                              return `${d.getHours().toString().padStart(2, "0")}:${d.getMinutes().toString().padStart(2, "0")}`;
                            })()}
                          </p>
                          <p className="text-sm text-on-surface-variant font-medium mt-1">
                            {schedule.toLocation || destination || "Điểm đến"}
                          </p>
                        </div>
                      </div>
                    </div>
      ...
      <section className="bg-surface-container-low py-24">
        <div className="max-w-7xl mx-auto px-6">
          <div className="mb-16">
            <h2 className="text-3xl font-black text-on-surface mb-2">Tại sao chọn BusGo?</h2>
      ...
      <section id="partners" className="py-24 bg-surface-container-low">
        <div className="max-w-7xl mx-auto px-6">
          <div className="mb-12">
            <h2 className="text-3xl font-black text-on-surface mb-2">Nhà xe đối tác</h2>
            <p className="text-on-surface-variant">Các nhà xe uy tín hợp tác với BusGo</p>
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="mb-12">
            ...
            {promotionsError && !promotionsLoading && (
              <div className="text-center text-red-500 py-6 font-medium bg-red-50 rounded-xl">
                {promotionsError}
              </div>
            )}
                  ...
                  <div
                    ...
                    onClick={() => {
                      setSelectedPromotion({
                        ...item,
                        imageUrl:
                          item.imageUrl || fallbackPromoImages[index % fallbackPromoImages.length],
                      });
                    }}
                  ...
                  >
                    <div className="h-40 w-full overflow-hidden relative bg-surface-container-low">
                      <img
                        ...
                        onError={(e) => {
                          e.target.src = fallbackPromoImages[index % fallbackPromoImages.length];
                        }}
                      />
                    ...
                    <div className="p-4 flex flex-col h-[100px]">
                      <h4
                        className="font-bold text-sm text-on-surface line-clamp-2 leading-relaxed mb-2"
                        title={item.title}
                      >
                        {item.title}
                      </h4>
