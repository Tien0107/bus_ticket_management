...
export const pageSurfaceClass = "min-h-screen bg-[#f6f8f5] px-4 py-4 lg:px-6";
      ...
      <div className={`mx-auto ${maxWidth}`}>
        <div className="mb-4 flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          <div className="min-w-0">
            ...
            <h1 className="text-2xl font-extrabold tracking-tight text-on-surface lg:text-3xl">
              {title}
            </h1>
            {description && (
              <p className="mt-1 max-w-3xl text-xs leading-5 text-on-surface-variant lg:text-sm">
                {description}
              </p>
            )}
          </div>
...
export function DangerButton({ icon, children, className = "", ...props }) {
  return (
    <button
      type="button"
      className={`inline-flex items-center justify-center gap-2 rounded-lg border border-red-200 bg-red-50 px-3.5 py-2 text-sm font-bold text-red-700 transition-colors hover:bg-red-100 disabled:cursor-not-allowed disabled:opacity-60 ${className}`}
      {...props}
    >
      {icon && <span className="material-symbols-outlined text-[20px]">{icon}</span>}
      {children}
  ...
  return (
    <div className="rounded-xl border border-outline-variant/30 bg-white p-4 shadow-sm">
      <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-3">
        <div className="min-w-0">
          <p className="min-h-[2.25rem] text-xs font-medium leading-5 text-on-surface-variant">
            {label}
          </p>
          <p className={`mt-0.5 leading-tight ${valueClass} font-extrabold text-on-surface`}>
            {value}
          </p>
        </div>
