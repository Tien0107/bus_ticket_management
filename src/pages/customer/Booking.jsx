...
export default function Booking() {
  ...
  const handleConfirmBooking = async () => {
    try {
      ...
      if (!bookingData.passengerInfo.name || !bookingData.passengerInfo.phone) {
        alert("Vui lòng điền đủ thông tin hành khách!");
        return;
      }
      ...
      const bookingPromises = bookingData.selectedSeats.map((seat) => {
        return createBooking({
            ...
            userFullName: bookingData.passengerInfo.name,
            phoneNumber: bookingData.passengerInfo.phone,
            email: bookingData.passengerInfo.email,
            userNote: bookingData.passengerInfo.note || "",
            couponId: bookingData.coupon?.id || undefined,
            couponDetailId: bookingData.coupon?.detailId || undefined,
        });
      });
