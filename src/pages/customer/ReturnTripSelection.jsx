...
export default function ReturnTripSelection({ outboundData, setReturnBookingData, setBookingPhase, onCancel }) {
  ...
  useEffect(() => {
    ...
    const fetchSchedules = async () => {
      try {
        setLoading(true);
        // Tìm chuyến về: Đảo ngược from và to
  ...
  const handleSelectSchedule = (schedule) => {
    setReturnBookingData({
      ...
      pickupId: null,
      dropoffId: null,
      passengerInfo: { name: "", phone: "", email: "", note: "" }, // Not heavily used for return but keep struct
      coupon: null,
      paymentMethod: "vnpay",
      totalPrice: 0,
      schedule: schedule
    ...
    <div className="bg-surface-container-low rounded-3xl p-8 border border-surface-container shadow-sm mb-8 animate-in fade-in zoom-in-95 duration-300">
      ...
      <div className="mb-8">
        ...
        <input 
          type="date" 
          value={date}
          min={outboundData.date} // Ngày về không thể trước ngày đi
