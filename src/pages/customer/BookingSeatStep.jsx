...
export default function BookingSeatStep({ bookingData, setBookingData, onNext }) {
  ...
  const price = 250000; // Giá cứng tạp thời vì API home chưa trả giá
  ...
  const handleSeatClick = (seat) => {
    ...
    setBookingData(prev => {
      ...
      const newSeats = isSelected 
        ? prev.selectedSeats.filter(s => s.seatNumber !== seat.seatNumber)
        : [...prev.selectedSeats, { id: seat.id, seatNumber: seat.seatNumber }];
      return { ...prev, selectedSeats: newSeats, totalPrice: newSeats.length * price };
    });
  };
  ...
  const handleDropoffChange = (d) => {
    setBookingData(prev => ({ ...prev, dropoffId: d.stationId, dropoffOrder: d.stopOrder, selectedSeats: [], totalPrice: 0 }));
  };
                  ...
                  {dropoffs.map(d => (
                    <li key={d.stationId} className="flex gap-3 cursor-pointer items-start" onClick={() => handleDropoffChange(d)}>
                      ...
                      <div>
                        ...
                        <p className="text-sm text-on-surface-variant mt-1 italic">{d.city}</p>
                      </div>
                    </li>
                  ))}
                </ul>
