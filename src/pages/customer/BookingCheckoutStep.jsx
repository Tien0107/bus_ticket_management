...
export default function BookingCheckoutStep({ bookingData, setBookingData, onBack, onConfirm }) {
  ...
  const applyCoupon = async () => {
    ...
    setCouponError("");
    try {
      const res = await checkCoupon({ code: couponInput.trim(), orderTotal: bookingData.totalPrice });
      const data = res.data;
      if (data && data.discountAmount) {
         setDiscountVal(data.discountAmount);
         setBookingData(p => ({
            ...p,
            coupon: { id: data.id || null, detailId: data.detailId || null, code: couponInput }
         }));
      } else {
          ...
          <section className="bg-surface-container-lowest p-8 rounded-xl shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="flex flex-col gap-2">
                ...
                <input name="name" value={bookingData.passengerInfo.name} onChange={handleChangeInfo} className="w-full bg-surface-container-low border-none rounded-xl p-3 focus:ring-2 focus:ring-primary/20 outline-none" placeholder="Nhập họ Tên" type="text" />
              </div>
              <div className="flex flex-col gap-2">
                <label className="text-sm font-semibold text-on-surface-variant">Số điện thoại</label>
                <input name="phone" value={bookingData.passengerInfo.phone} onChange={handleChangeInfo} className="w-full bg-surface-container-low border-none rounded-xl p-3 focus:ring-2 focus:ring-primary/20 outline-none" placeholder="09xxxx" type="tel" />
              </div>
              <div className="md:col-span-2 flex flex-col gap-2">
                <label className="text-sm font-semibold text-on-surface-variant">Vị trí điểm đón cụ thể</label>
                <input name="note" value={bookingData.passengerInfo.note} onChange={handleChangeInfo} className="w-full bg-surface-container-low border-none rounded-xl p-3 focus:ring-2 focus:ring-primary/20 outline-none" placeholder="Ghi chú yêu cầu..." type="text" />
              </div>
            </div>
          </section>
              ...
              <label className="flex items-center p-4 rounded-xl border border-surface-container hover:border-primary cursor-pointer transition-all">
                ...
                <div className="flex-1 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="font-semibold text-on-surface">Thanh toán tại quầy</span>
                  </div>
                </div>
              </label>
            </div>
