...
export default function TicketDetail() {
  ...
  // Extract variables with defaults if missing
  const departureCity = ticket.startCity || "Sài Gòn";
  const arrivalCity = ticket.endCity || "Đà Lạt";
  const departureStation = ticket.startStation || "Bến xe Miền Đông";
  const arrivalStation = ticket.endStation || "Bến xe Đà Lạt";
  const date = ticket.departureDate ? new Date(ticket.departureDate).toLocaleDateString() : "25/12/2026";
  const time = ticket.departureTime || "07:00";
  const seatNumbers = ticket.seatId || (ticket.selectedSeats && ticket.selectedSeats.map(s => s.seatNumber).join(", ")) || "A12, A13";
  const driverPhone = ticket.driverPhone || "Chưa cập nhật";
  const busCompany = ticket.companyName || "Phương Trang";
  const busPlate = ticket.licensePlate || "51B-293.44";
  const price = (ticket.totalPrice || 250000).toLocaleString('vi-VN') + 'đ';
  const qrCodeData = ticket.qrCode || `BG-${ticketId}`;
        ...
        <div className="relative bg-surface-container-lowest rounded-2xl overflow-hidden shadow-[0_12px_32px_rgba(26,28,28,0.06)] border border-surface-container/50">
          ...
          <div className="p-8 flex flex-col items-center">
            ...
            <div className="mt-6 w-full pt-6 bg-surface-container-low rounded-xl flex justify-between items-center px-6 pb-6 border-t border-white/50">
              ...
              <div className={`flex items-center gap-2 ${isCancelled ? 'text-error' : (ticket.status === 'PENDING' ? 'text-secondary' : 'text-primary')}`}>
                <span className="material-symbols-outlined text-lg" style={{fontVariationSettings: "'FILL' 1"}}>
                  {isCancelled ? 'cancel' : (ticket.status === 'PENDING' ? 'schedule' : 'verified')}
                </span>
                <span className="text-[11px] font-bold uppercase tracking-widest">
                  {isCancelled ? 'Đã hủy' : (ticket.status === 'PENDING' ? 'Chưa thanh toán' : 'Đã thanh toán')}
                </span>
              </div>
            </div>
          </div>
